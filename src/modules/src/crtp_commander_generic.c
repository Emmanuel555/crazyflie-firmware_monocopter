...
enum packet_type {
  ...
  fullStateType     = 6,
  positionType      = 7,
  transitionType    = 8,  // added by xinyu for transition project
};
...
// transition decode: 5 data: x, y, z, yaw,roll ######################
  struct transitionPacket_s {
    float cai_x;
    float cai_y;
    float cai_z;
    float cai_yaw;
  ...
  static void transitionDecoder(setpoint_t *setpoint, uint8_t type, const void *data, size_t datalen)
  {
    const struct transitionPacket_s *value = data;

    ASSERT(datalen == sizeof(struct transitionPacket_s));

    setpoint->mode.x = modeAbs;
    setpoint->mode.y = modeAbs;
    setpoint->mode.z = modeAbs;

    setpoint->position.x = value->cai_x;
    setpoint->position.y = value->cai_y;
    setpoint->position.z = value->cai_z;    

    setpoint->mode.roll = modeAbs;
    setpoint->mode.yaw = modeAbs;

    setpoint->attitude.yaw = value->cai_yaw;
    setpoint->attitude.roll = value->cai_roll;
  }  // transition decode ends #########################
...
const static packetDecoder_t packetDecoders[] = {
  ...
  [transitionType]    = transitionDecoder,  // add by cai ############
};
