 ...
 *
 * power_distribution_monocopter.c - Crazyflie stock power distribution code
 */
...
static void equalpowerDistribution(const control_t *control, motors_thrust_uncapped_t* motorThrustUncapped)
{
  motorThrustUncapped->motors.m1 = control->thrust;
  motorThrustUncapped->motors.m2 = control->thrust_2;
  motorThrustUncapped->motors.m3 = control->thrust_3;
  motorThrustUncapped->motors.m4 = control->thrust_4;
}
...
{
  equalpowerDistribution(control, motorThrustUncapped);  
}
...
{
  bool isCapped = false;
  motorPwm->motors.m1 = limitThrust(motorThrustBatCompUncapped->motors.m1, 0, UINT16_MAX, &isCapped); // 
  motorPwm->motors.m3 = limitThrust(motorThrustBatCompUncapped->motors.m3, 0, UINT16_MAX, &isCapped); // 
  motorPwm->motors.m2 = limitThrust(motorThrustBatCompUncapped->motors.m2, 0, UINT16_MAX, &isCapped); // 
  motorPwm->motors.m4 = limitThrust(motorThrustBatCompUncapped->motors.m4, 0, UINT16_MAX, &isCapped); // 
  return isCapped;
}
